...
void MakeReflectionImage( void )
{
    ...
    vEyePos[0] = eyePos[0];
    vEyePos[1] = eyePos[1];
    vEyePos[2] = 2 * TABLETOP_Z - eyePos[2];
    ...
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - vEyePos[1], TABLETOP_Y2 - vEyePos[1], TABLETOP_X1 - vEyePos[0], TABLETOP_X2 - vEyePos[0], TABLETOP_Z - vEyePos[2], eyeDistance + SCENE_RADIUS);
    
    // Step 3
    
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(vEyePos[0], vEyePos[1], vEyePos[2], vEyePos[0], vEyePos[1], eyePos[2] , 1, 0, 0);
    // Step 4
...
void MyDisplay( void )
{
    ...
    // Set world-space positions of the two lights.Z`
    glLightfv( GL_LIGHT0, GL_POSITION, light0Position );
    glLightfv( GL_LIGHT1, GL_POSITION, light1Position );
...
// This texture object is for the earth texture map.
    ...
    if (ReadImageFile(earthTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// Display user instructions in console window.
    ...
    printf("Press 'P' to toggle earth's animation.\n");
...
// interpolated to the newldy created vertices.
/////////////////////////////////////////////////////////////////////////////
...
void DrawDonutEarth(void) {
    ...
    glPushMatrix();
    glTranslatef(TABLETOP_X1 / 2, - TABLETOP_Y1 / 2, TABLETOP_Z * 2);
    glRotatef(90, 0, 1, 0);
    glScalef(0.3, 0.3, 0.3);
    glRotatef(rotationAngle, 1, 0.5, 0.25);
    drawTorus(0.3, 0.4, 30, 30, earthTexObj);
