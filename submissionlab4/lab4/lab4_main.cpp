...
static constexpr std::string_view outImageFile1 = "img_r0s.png";
...
void DefineScene2(Scene& scene, int imageWidth, int imageHeight)
{
    ...
    whitish.k_r = Color(0.9f, 0.9f, 0.9f);
    whitish.k_rg = Color(0.2f, 0.2f, 0.2f);
    whitish.n = 128.0f;

    // Insert into scene materials vector.
    scene.materials = { lightRed, lightGreen, lightBlue, yellow, gray, whitish };
    ...
    auto leftVertPlane = new Plane(1.0, 0.0, 0.0, 0.0, scene.materials[4]); // Left vertical plane.
    auto rightVertPlane = new Plane(0.0, 0.0, 1.0, 0.0, scene.materials[4]); // Right vertical plane.
    auto bigSphere = new Sphere(Vector3d(65.0, 12.5, 65.0), 17.0, scene.materials[5]); // Big sphere.
    auto smallSphere1 = new Sphere(Vector3d(90.0, 15.0, 40.0), 15.0, scene.materials[3]); // small sphere.
    auto smallSphere2 = new Sphere(Vector3d(40.0, 15.0, 90.0), 15.0, scene.materials[2]);

    // Top right pyramid face
    auto topRight = new Triangle(Vector3d(15.0, 0.0, 15.0), Vector3d(32.25, 25.75, 32.25),
        Vector3d(50.0, 0.0, 15.0), scene.materials[3]);

    // Top left pyramid face
    auto topLeft = new Triangle(Vector3d(15.0, 0.0, 50.0), Vector3d(32.25, 25.75, 32.25),
        Vector3d(15.0, 0.0, 15.0), scene.materials[3]);

    // bot right pyramid face
    auto botRight = new Triangle(Vector3d(50.0, 0.0, 15.0), Vector3d(32.25, 25.75, 32.25),
        Vector3d(50.0, 0.0, 50.0), scene.materials[3]);

    // bot left pyramid face
    auto botLeft = new Triangle(Vector3d(50.0, 0.0, 50.0), Vector3d(32.25, 25.75, 32.25),
        Vector3d(15.0, 0.0, 50.0), scene.materials[3]);

    

    scene.surfaces = { horzPlane, leftVertPlane, rightVertPlane,
                       bigSphere, smallSphere1, smallSphere2, topRight, botRight, botLeft, topLeft };


    // Define camera.
    scene.camera = Camera(Vector3d(200.0, 120.0, 200.0),  // eye
        Vector3d(55.0, 27.5, 55.0),  // lookAt
        Vector3d(0.0, 1.0, 0.0),  //upVector
        (-1.0 * imageWidth) / imageHeight,  // left
        (1.0 * imageWidth) / imageHeight,  // right
        -1.0, 1.0, 3.0,  // bottom, top, near
