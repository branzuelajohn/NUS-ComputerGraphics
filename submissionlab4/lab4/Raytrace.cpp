...
// Compute for shadow if hasShadow is true.
    ...
    for (int i = 0; i < scene.ptLights.size(); i++) {
        float kShadow = 1.0;
        Vector3d L = (scene.ptLights[i].position - nearestHitRec.p).makeUnitVector();
        if (hasShadow) {
            ...
            for (int j = 0; j < scene.surfaces.size(); j++) {
                if (scene.surfaces[j]->shadowHit(shadow, DEFAULT_TMIN, tmax)) {
                    kShadow = 0.0;
                    break;
                }
            }
        }
        result += kShadow * computePhongLighting(L, N, V, nearestHitRec.material, scene.ptLights[i]);
    }
    ...
    // Add to result the reflection of the scene.
        ...
        //*********** WRITE YOUR CODE HERE **************
        //***********************************************
    if (reflectLevels == 0) return result;

        Vector3d reflectedRayDirection = mirrorReflect(V, N);
        Ray reflectionRay(nearestHitRec.p, reflectedRayDirection);
        return result + nearestHitRec.material.k_rg * Raytrace().TraceRay(reflectionRay, scene, reflectLevels - 1, hasShadow);
}
