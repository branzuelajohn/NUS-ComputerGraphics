...
void MyReshape(int w, int h)
{
    ...
    //****************************
    if (w <= h)
        gluOrtho2D(-2.0, 2.0, -2.0 * (GLfloat)h / w, 2.0 * (GLfloat)h / w);
    else
        gluOrtho2D(-2.0 * (GLfloat)w / h, 2.0 * (GLfloat)w / h, -2.0, 2.0);
...
void UpdateAllDiscPos(void)
{
    for (int i = 0; i < numDiscs; i++)
    {
        ...
        else if (disc[i].pos[0] <= disc[i].radius)
        {
            ...
            disc[i].pos[0] = disc[i].radius; // realign in case disc move out of frame
        }
        ...
        if (disc[i].pos[1] + disc[i].radius >= winHeight)
        {
            ...
            disc[i].pos[1] = winHeight - disc[i].radius; // Realign in case disc move out of frame
        }
        // Bottom window frame
        else if (disc[i].pos[1] <= disc[i].radius)
        {
            disc[i].speed[1] = -disc[i].speed[1];
            disc[i].pos[1] = disc[i].radius; // realign incase disc move out of frame
        }
