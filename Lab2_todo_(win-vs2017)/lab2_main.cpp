...
void DrawOneCar( float bodyColor[3] )
{
    ...
    int innerRadius = CAR_LENGTH / 16;
    int outerRadius = CAR_LENGTH / 8;
    int radiusOfTube = (outerRadius - innerRadius) * 2;

    //Bottom of the car (Rectangle)
    glPushMatrix();
    glTranslatef(0, 0, CAR_HEIGHT / 2);
    glScalef(1, CAR_WIDTH /  CAR_LENGTH, (0.5 * CAR_HEIGHT) / CAR_LENGTH);
    ...
    glScalef(0.6 , CAR_WIDTH / CAR_LENGTH, (0.5* CAR_HEIGHT) / CAR_LENGTH);
    glutSolidCube(CAR_LENGTH);
    glPopMatrix();
    ...
    // Front Left Tyre
    glPushMatrix();
    glTranslatef(CAR_LENGTH / 3, (CAR_WIDTH + radiusOfTube) / 2, outerRadius);
    glRotatef(90, 1, 0, 0);
...
void DrawAllCars( void )
{
    for ( int i = 0; i < NUM_CARS; i++ )
    {
        ...
        //****************************
        // 1. draw the car
        // 2. translate the car to the circumference of the big circle
        // 3. rotate the car to its angular position
        // 4. rotate the great circle about the xz axis
        CarType* c = &car[i];
        //glVertex2dv(c->xzAxis);
        double theta = c->rotAngle;
        glRotatef(theta, c->xzAxis[0], 0, c->xzAxis[1]);
        glPushMatrix();
...
void MyDisplay(void)
{
    ...
    gluPerspective(VERT_FOV, (double)winWidth / winHeight,
        EYE_MIN_DIST - CLIP_PLANE_DIST, CLIP_PLANE_DIST * CLIP_PLANE_DIST); 
